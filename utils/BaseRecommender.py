...
class BaseRecommender(object):
    ...
    def recommend(self, userId, at=None, remove_seen_flag=True, items_to_compute=None,
        ...
        if at is None:
            at = self.URM_train.shape[1] - 1

        # Compute the scores using the model-specific function
        # Vectorize over all users in user_id_array
        scores_batch = self._compute_item_score(userId, items_to_compute=items_to_compute)

        for user_index in range(len(userId)):
        ...
        relevant_items_partition = (-scores_batch).argpartition(at, axis=1)[:, 0:at]

...
def check_matrix(X, format_='csc', dtype=np.float32):
    """
    ...
    if format_ == 'csc' and not isinstance(X, sps.csc_matrix):
        return X.tocsc().astype(dtype)
    elif format_ == 'csr' and not isinstance(X, sps.csr_matrix):
        return X.tocsr().astype(dtype)
    elif format_ == 'coo' and not isinstance(X, sps.coo_matrix):
        return X.tocoo().astype(dtype)
    elif format_ == 'dok' and not isinstance(X, sps.dok_matrix):
        return X.todok().astype(dtype)
    elif format_ == 'bsr' and not isinstance(X, sps.bsr_matrix):
        return X.tobsr().astype(dtype)
    elif format_ == 'dia' and not isinstance(X, sps.dia_matrix):
        return X.todia().astype(dtype)
    elif format_ == 'lil' and not isinstance(X, sps.lil_matrix):
        ...
        X.eliminate_zeros()
        return check_matrix(X, format_=format_, dtype=dtype)
    else:
...
def similarityMatrixTopK(item_weights, k=100, verbose=False):
    ...
    if sparse_weights:
        item_weights = check_matrix(item_weights, format_='csc', dtype=np.float32)
    else:
    ...
    W_sparse = sps.csc_matrix((data, rows_indices, cols_indptr), shape=(n_items, n_items), dtype=np.float32)
